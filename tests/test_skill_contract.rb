...
class SkillContractTest < Minitest::Test
  ...
  def test_tests_and_current_product_spec_are_distributed_but_generated_material_is_ignored
  ...
  def test_rule_ownership_and_conflict_precedence_are_explicit
    ...
    assert_includes policy, "只定义产品目标、组件边界和规则归属"
  ...
  def test_all_profiles_share_one_managed_china_domain_baseline
    ...
    [readme, skill, policy_doc].each do |document|
  ...
  def test_saved_profile_bounds_diagnostics_repairs_and_regression_checks
    ...
    [readme, skill, policy].each { |document| assert_includes document, "档位" }
  ...
  def test_subscription_update_uses_platform_native_client_refresh
    ...
    assert_includes readme, "`-BeginSafeUpdateRefresh -Json`"
    assert_includes readme, "`-VerifySafeUpdate -RefreshConfirmed -Json`"
    ...
    assert_includes safe_update, ".\\scripts\\install_windows.cmd -BeginSafeUpdateRefresh -Json"
  ...
  def test_safe_update_requires_provider_switch_confirmation_before_the_first_attempt
    ...
    [documents[0], documents[1], documents[3]].each do |document|
  ...
  def test_diagnostics_starts_from_a_general_task_contract_evidence_inventory_and_authority_boundary
    ...
    public_contract = [readme, skill, policy].join("\n")
  ...
  def test_diagnostics_never_launches_clash_client_as_an_inspection_probe
    ...
    [skill, policy, agent_instructions].each do |source|
