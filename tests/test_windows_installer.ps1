...
Add-Type -TypeDefinition 'namespace ClaudeEasy { public static class SendInputNative { public static string RuntimePath; public static string AppHome; public static string DispatchLogPath; public static int FirstDelayMilliseconds; public static bool FailRestoreDispatch; private static int SendCount; private static bool IsRestoreDispatch() { if (string.IsNullOrEmpty(AppHome)) { return false; } string manifestPath = System.IO.Path.Combine(AppHome, "claude-easy-safe-update.json"); if (!System.IO.File.Exists(manifestPath)) { return false; } return System.IO.File.ReadAllText(manifestPath).IndexOf("safe_update_runtime_recovery", System.StringComparison.Ordinal) >= 0; } public static bool Send(System.UInt16[] keys) { if (keys == null || keys.Length != 4 || keys[0] != 0x11 || keys[1] != 0x12 || keys[2] != 0x10 || keys[3] != 0x87) { return false; } int count = System.Threading.Interlocked.Increment(ref SendCount); if (!string.IsNullOrEmpty(DispatchLogPath)) { System.IO.File.AppendAllText(DispatchLogPath, count.ToString() + "\n"); } bool isRestore = IsRestoreDispatch(); if (isRestore && FailRestoreDispatch) { return false; } string path = RuntimePath; int delay = isRestore ? 200 : FirstDelayMilliseconds; var delayThread = new System.Threading.Thread(delegate() { System.Threading.Thread.Sleep(delay); System.IO.File.AppendAllText(path, "\n# simulated refresh\n"); }); delayThread.IsBackground = true; delayThread.Start(); return true; } } }' -ErrorAction Stop | Out-Null
