...
- macOS 订阅下载必须使用 Foundation 原生请求，并从当前运行的 ClashX Meta 动态生成客户端身份；不得使用 curl，也不得固定或伪造 User-Agent。Windows 订阅下载不得添加、固定或伪造 User-Agent。两端都必须发送 `Accept-Language: zh-CN,zh;q=0.9`，并在任一原代理组或节点选择无法恢复时拒绝更新。
