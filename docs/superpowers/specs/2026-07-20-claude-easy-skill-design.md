...
| 3｜Claude/Claude Code | Claude、Claude Code 和更强的泄漏防护 | 继承档位 2 | DNS 分流、AI 分组与规则、局域网与国内 UDP 分流、其余 UDP/WebRTC 防护、区域指纹检查 |
...
档位 3 使用本地 `assets/claude-region-check.html`，在实际打开 Claude 的同一浏览器中比较修改前后区域信号。页面先披露固定 STUN 服务、正常网页出口对照及其可见的公网 IP，再由明确按钮触发；WebRTC 候选只留在本地，CSP 只允许固定对照端点。WebRTC 项只有明确暴露本地网络地址时计分；公网出口不同、没有公网候选或无法完成对照都不计分，因为这些情况不能单独证明 WebRTC 绕过代理；不做国家归属查询。正式支持 macOS Safari/Chrome 和 Windows Edge/Chrome。它提供十项参考信号、风险分档和未知权重，不是 Claude 官方判断，也不能作为 DNS、WebRTC、实时分流或 Claude 联网的通过条件。具体信号、权重、授权和恢复规则只在 `profiles-and-patch.md` 定义。
...
配置解析、外部进程调用和可执行扩展脚本都必须先把输入转换成可完整判断的表示，再进入递归解析、命令解释器或受管代码组合。检查器不能依赖异常兜底，也不能只识别已知危险样例。

- macOS YAML 在 Psych 递归物化前迭代检查节点数量和嵌套深度；超出预算按无效配置处理，不进入递归访问器。
- Windows 直接执行 Mihomo；仅测试兼容所需的命令脚本必须拒绝命令解释器元字符，并关闭延迟环境变量展开。
- Windows `Script.js` 词法检查必须识别正则字面量；代码区出现 Unicode 转义时拒绝组合，避免标识符检查基于非规范拼写。

不采用三种窄修：只捕获 `SystemStackError` 会让资源消耗继续发生；只过滤某个路径字符会遗漏同类命令解释；只搜索一个转义后的保留名会遗漏其他等价标识符。所有拒绝都发生在文件事务写入前，并保持原文件不变。
