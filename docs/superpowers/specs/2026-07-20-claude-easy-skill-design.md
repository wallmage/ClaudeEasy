...
订阅更新只由用户显式触发，不是后台监听。两端都先为全部远程订阅创建更新前备份，再用 `curl -q --config -` 自动下载，且不得添加、固定或伪造 User-Agent。Windows 以文件事务直接覆盖下载结果后结束；macOS 按保存档位生成并校验补丁候选，整批写入后加载当前订阅、完成档位运行检查，并再次确认自动更新关闭，失败时按事务记录恢复。
