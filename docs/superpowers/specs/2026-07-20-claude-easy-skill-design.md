...
订阅更新只由用户显式触发，不是后台监听。两端都先为全部远程订阅创建更新前备份，再用 `curl -q --config -` 自动下载，且不得添加、固定或伪造 User-Agent。两端都按保存档位检查候选，整批写入后加载当前订阅、完成补丁与档位运行检查，并再次确认自动更新关闭；Windows 通过 Clash Verge Rev 的受管重新激活入口执行全局脚本，任一步失败都按事务记录恢复原订阅和原运行配置。
