...
订阅更新只由用户显式触发，不是后台监听。两端都先为全部远程订阅创建更新前备份。macOS 用 Foundation 原生请求自动下载；Windows 只通过 Clash Verge Rev 的“更新所有订阅”执行客户端原生刷新，有 Computer Use 时由代理操作，没有时给用户相同步骤并在用户确认完成后继续。两端更新后都按保存档位确认补丁、自动更新关闭状态，并完成相同的客户端动作和档位验收；任一步失败都按更新前备份和平台事务规则恢复。
