...
先寻找订阅已有的可选 AI 分组。名称匹配策略文件中的 AI 组名称，或包含独立的 `AI`、`OpenAI`、`人工智能` 时，视为已有 AI 分组。macOS 与 Windows 都直接复用已有分组，只补全规则，并保持分组类型、成员、顺序、图标和当前选择。只有名称、受管规则以及现有节点和提供者清单都完整符合 ClaudeEasy 生成结构的分组，才允许按当前代理节点和提供者更新成员。
...
macOS 与 Windows 都保持用户原有 AI 分组的成员。ClaudeEasy 自己新建或能够确认由旧版创建的 AI 分组会填入全部可用节点和代理提供者。补丁不得调用控制器切换节点，也不得替用户选择台湾、日本或任何家宽节点。

AI 分组的用途是分开普通流量和 AI 流量：主代理组可以选择低倍率节点用于浏览、下载和视频，AI 分组可以由用户单独选择信誉更好的家宽节点。补丁只提供完整选项，不替用户决定。

聊天中说明家宽通常比数据中心节点更适合 AI 服务；首选台湾家宽，没有家宽时选择台湾普通节点，没有合适的台湾节点时次选日本并仍以家宽优先，时区次选东京。台湾方案同时建议台北时区、台湾系统地区和“繁體中文（台灣）”浏览器语言，使代理出口、时区、地区和浏览器语言尽量统一；台北与中国大陆同为东八区，不改变电脑显示的实际时间。建议只写在聊天中，最终选择完全交给用户。
...
2. `lan_udp_direct_rules` 命中的局域网、链路本地和组播 UDP 走 `DIRECT`。
3. 共同国内域名规则提供器命中的流量走 `DIRECT`。
4. 同时命中 UDP 类型和受管中国 IP 规则库的流量走 `DIRECT`。
5. `NETWORK,UDP,<AI 分组>`：其余 UDP 走 AI 分组。
6. `NETWORK,UDP,REJECT`：所选节点不支持 UDP 时拒绝，不改走本地直连。

局域网规则只使用 `policy.json` 明确列出的目标网段，不能直接采用包含 `198.18.0.0/15` 的通用 private 规则库；该网段是 Mihomo 默认 Fake-IP 范围，把它当作局域网会让大量无法确认的 UDP 误走直连。局域网规则必须放在其余 UDP 规则之前，不能依赖订阅原有的私网规则，因为后者会排在受管 UDP 规则之后。

补丁输出必须把上述受管规则写成连续且顺序固定的整体，二次转换必须保持不变。备份恢复候选中任意受管规则缺失、换序或被其他规则插入都必须拒绝，不能只确认各条规则分别存在；两个国内规则提供器的类型、行为、格式、地址、路径、更新周期、下载代理和大小限制也必须与当前策略及配置中的路由分组一致。策略 v2 缺少或损坏任一必需字段时，两端都保持原配置不变。
