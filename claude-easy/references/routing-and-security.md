...
第一条的目标使用 AI 分组，不创建安全代理分组。把两条都放在规则列表开头，确保更靠后的 `NETWORK,udp,DIRECT`、`DST-PORT,3478,DIRECT`、进程规则和任意 `RULE-SET` 都不能先放行 UDP。除受管 AI 规则的同键冲突和旧版受管 UDP 规则外，已有规则保持原目标和相对顺序。
