...
脚本的 JSON、退出码、状态、检查项和逐订阅结果供代理内部判断，不直接复制给用户。用户可见说明遵守 [SKILL.md](../SKILL.md) 的最高沟通原则：完成时先给一句结果，再用最多三项日常语言说明已经处理和验证的内容；没有用户必须采取的动作就不附技术细节。

```text
已经全部设置好，档位 3 的补丁和检查都已通过。
```
