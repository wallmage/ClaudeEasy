...
冲突按以下顺序处理：先遵守 [SKILL.md 共同安全边界](../SKILL.md#共同安全边界)，用户当次明确授权和用途档位仅在边界内限定可做范围；随后具体场景规则优先于通用规则，平台规则优先于跨平台摘要，事务安全规则优先于一般失败恢复规则。仍无法唯一判断时停止对应写入并报告规则冲突，不自行挑选较宽松解释。
...
**Claude/Anthropic 远程域名永久禁测：** 相关网站、API 和域名一律不打开、不请求、不测试。禁止通过浏览器、Computer Use、curl、脚本、DNS、WebRTC、分流验证或任何其他方式向这些域名产生测试流量；用户当次要求也不能覆盖。只允许静态检查配置，AI 联网与分流验收只测试 ChatGPT、Gemini 和 Grok。区域指纹验收按 [profiles-and-patch.md](profiles-and-patch.md#patch-验证标准) 使用 here.now 页面，不请求 Claude/Anthropic 域名；已保存档位为 3 时，任何配置任务都必须在收尾运行一次区域指纹测试，每轮调整后再运行一次，直到低风险通过。
