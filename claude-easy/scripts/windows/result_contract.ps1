...
function Protect-ClaudeEasyResultText([object]$Value) {
    ...
    $text = [regex]::Replace($text, '(?i)(?<![A-Za-z0-9])Bearer\s+\S+', 'Bearer [已隐藏]')
    $text = [regex]::Replace($text, '(?i)(?<![A-Za-z0-9])(password|passwd|secret|token|uuid|private[-_ ]?key|controller[-_ ]?key)\s*[:=]\s*\S+', '$1=[已隐藏]')
