...
if [ "$SAFE_UPDATE" -eq 1 ]; then
  if [ -n "$CUSTOM_PROFILE_DIR" ]; then
    if ! run_committing_profile_operation \
      ...
      finish_profile_operation_result_failure
      if [ "$JSON_OUTPUT" -eq 1 ]; then
        finish_json_child_failure "$child_json" failed safe_update_failed "订阅更新失败。" safe_update
      else
        say "订阅更新失败。"
        finish 1 failed safe_update_failed "订阅更新失败。" safe_update
      fi
    fi
  else
    if ! run_committing_profile_operation \
