...
3. **主动报告变动明细。** macOS 与 Windows 的所有检查、比较、更新、配置、修复和恢复，都必须在最终回复中按对象说明本轮发现或实际发生的变化，无需用户追问。逐项写清对象名称、变化内容、可确认的前后差别和实际处理结果；区分“发现但未修改”“已应用”“失败”和“已恢复”。有变化时不能只报数量、笼统说“已更新”，也不能用验收通过清单代替变动明细。无变化对象可合并列名；全部无变化时说明检查范围和无变化结论。没有比较基准时说明只能确认当前状态，不把未知说成无变化。订阅专有明细按 [safe-update-and-recovery.md](references/safe-update-and-recovery.md) 执行；通用电脑流程不因此读取网络策略。
4. **简短不能省略变动明细。** 先说结果，再给明细和必要的验收结论；篇幅由实际变化决定。隐藏工具调用、重试、日志和内部状态，不隐藏第 3 条要求的内容。敏感值不展示，用名称和“连接参数已变更”等描述代替；订阅链接、服务器地址、端口、密码、UUID 和密钥不得进入报告。确实未达到用户目标时，如实说明影响和必要行动，不虚报完成，也不追加无关提醒。
