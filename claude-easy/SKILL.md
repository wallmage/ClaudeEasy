...
只有用户明确要求“请帮我安全更新全部订阅”或同义请求时执行。先创建全部远程订阅的清单、哈希和版本化备份，再通过客户端界面一次触发“立即执行安全更新”；不得检查或操作服务商后台、登录网站或改后台开关。随后逐份确认本轮刷新证据、YAML、Mihomo、代理组、当前策略和自动更新仍关闭；一份失败时按平台事务规则保持整批一致。逐一报告哪些订阅下载与校验成功、哪些失败，不能把文件已下载写成已生效。
