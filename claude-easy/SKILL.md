...
3. **共同安全边界允许的界面操作由代理完成。** 可靠脚本或结构化接口优先；必须点击正常应用窗口时使用当前会话实际提供的电脑操控（Computer Use）。用户明确要求更新、修复、配置、恢复或执行其他完整流程时，该授权包含 Skill 与本轮已读策略明确披露、且属于该流程固定步骤的界面操作、联网测试和数据传输；包括 DNS、WebRTC 与本地区域指纹测试按既有披露向测试服务发送公网 IP。不得把同一流程拆到每个页面或验收项重复询问；用户再说“直接操作”“直接做完”“不要停”或同义表达时更不得重复询问。该授权不扩展到流程外动作，也不覆盖工作台、操作系统或高风险动作实际触发的强制确认。
...
- **绝对禁止代理切换节点，任何情况都不允许。** 不得通过界面、控制器、API、脚本、配置或改变代理组选择直接或间接改换用户当前节点；不得先切换再恢复。只能读取当前选择和证据；怀疑节点故障时，按 [节点建议](references/routing-and-security.md#节点建议) 向用户建议，由用户自己操作。
- **AI 工作台和工具只允许查看，绝对禁止操作。** 包括 Claude、Claude Code、Codex、ChatGPT、Cursor 等桌面应用、网页、终端交互会话及内置工具。Computer Use 仅可读取已有画面、截图和界面状态；不得点击、输入、滚动、导航、打开或刷新 AI 页面、切换会话、新建、Fork、重试、发送、继续、停止、关闭或重启。不得用 API、命令或其他工具绕过界面禁令操作 AI 会话、任务或进程。需要交互才能取得证据时，由用户自己操作；代理继续只读取日志和已有状态，不把任务副本当作无影响的测试环境。缺少本轮证据就标记未验证，不为验收而触碰工作台。
