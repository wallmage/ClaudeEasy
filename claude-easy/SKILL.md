...
3. **主动报告变动明细。** macOS 与 Windows 的检查、比较、更新、配置、修复和恢复，按对象说明变化、可确认的前后差别和处理结果，区分“发现但未修改”“已应用”“失败”和“已恢复”。故障任务把原因相关发现放入下文 Root Cause，实际改动和复测放入 Fix Plan，不另列检查清单或无变化对象凑报告。其他任务全部无变化时说明检查范围和无变化结论。没有比较基准时不把未知说成无变化。订阅专有明细按 [safe-update-and-recovery.md](references/safe-update-and-recovery.md) 执行；通用电脑流程不因此读取网络策略。
4. **只保留帮助判断或行动的信息。** 进度只说原因判断或修复行动的新进展；最终回复遵守当前任务的交付要求。工具调用、重试过程和原始日志留在内部，支持原因判断的关键证据用一句话解释。敏感值不展示，用名称和“连接参数已变更”等描述代替；订阅链接、服务器地址、端口、密码、UUID 和密钥不得进入报告。不虚报完成，不追加无关提醒。
5. **过程问题由代理处理。** 工具报错、重试和中间状态不逐项播报；有安全的下一步就继续。确实需要用户参与时，只给一个明确动作，不让用户阅读排查记录或选择技术方案。

### 故障交付：Root Cause 与 Fix Plan

用户报告故障就默认需要原因与修复办法，无需再追问“为什么”。分析、复核和修复的最终回复只包含以下两项：

- **Root Cause（原因）：** 指出哪个环节因什么机制导致原始问题，并附最关键的依据。复述症状、翻译错误、确认“确实断线”或罗列检查结果都不是原因。查到哪层说到哪层；无法确定时直说“原因未确定”，给出已有证据支持的范围和阻止进一步定位的具体缺口，不编造唯一原因，也不堆砌可能性。
- **Fix Plan（修复方案）：** 针对上述原因给出具体对象、最小动作和成功判据。修复请求中，已授权且可执行的动作继续做完，报告实际改动、复测结果和剩余步骤；纯分析请求只给方案。原因未确定时，写明下一项能区分候选的取证、不同结果对应的处理；能自行取证就继续，不能用“继续观察”或“建议排查”收尾。临时绕过注明适用条件和局限，不冒充已验证的修复。
