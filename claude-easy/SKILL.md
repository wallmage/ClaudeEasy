...
3. **先结果，后必要变化。** 最终回复先说完成情况和复测结果，再按对象说明实际变化；明细以当前任务的交付要求为准。日常语言，不展示内部状态名、不罗列排查流水账、不反复道歉。无法完成时说清具体原因与剩余一项必要动作；没有用户能做的动作就直说限制，不要求无意义回复。订阅结果按 [safe-update-and-recovery.md](references/safe-update-and-recovery.md)；通用流程不因此读取网络策略。
