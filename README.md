...
- **Windows**：先运行 `.\claude-easy\scripts\install_windows.cmd -SnapshotProfiles` 核对每个远程订阅与本地文件一一对应、建立脱敏清单并逐份备份（存在尚未验收的清单时必须先完成验收，不会覆盖）；再通过 Clash Verge Rev 界面的“全部更新”处理，没有该入口时逐份触发，但必须覆盖清单中的每一份。随后运行 `-VerifySafeUpdate`，确认已安装的受管全局脚本与当前安装包及保存档位一致；受管区块外只允许空白或注释，出现可执行前缀、后缀或额外 `main` 时必须拒绝验收。重装会在通过组合检查后把区块外的旧代码迁入专用原脚本边界并保留。脚本检查通过后，再逐份确认 `proxy-groups` 是非空的块状或行内列表，并做 YAML、Mihomo 和自动更新状态检查；缺少代理组或列表为空时整批恢复，不能误报成功。若 `profiles.yaml` 的映射未变但订阅文件缺失，验收会从清单绑定的可信备份原子重建缺失文件，并与清单删除一起提交；同一路径重新出现时不覆盖，清单也会保留。失败时在独占文件期间再次核对 SHA-256，只恢复没有再次变化的文件——发现订阅又被刷新时不覆盖新内容，保留清单并明确报告。整批恢复与删除本次清单属于同一个可恢复事务；中断后不能拿已经恢复的旧订阅重新验收并误报更新成功。文件恢复后仍会单独检查运行内核，不会把文件恢复冒充运行状态恢复；档位 3 的全局脚本会在每份订阅随后加载时重打补丁。
...
- macOS 的普通安装、安全更新、安全卸载以及变更型 Ruby 命令，从读取用途档位到最终提交共用同一个跨进程操作锁。子流程必须同时验证继承的锁描述符、固定锁文件身份和锁仍被实际持有；单独伪造环境标志不能跳过互斥。另一个写操作已经进行时，新操作等待最多 5 秒后安全退出并报告 `operation_in_progress`；卸载最终核对后不会再有并发安装发布新档位，因此不会按路径误删刚生成的状态。卸载在删除后被强制中断时，下一次安装或安全更新必须在同一把锁内先完成该卸载；结果无法验证时保留恢复状态并停止，不能把半完成卸载当成首次安装继续。
