...
档位 3 会在修改前后各用一次系统默认浏览器打开 [Claude 区域指纹检测页](https://fuck-claude.vercel.app/zh/)，由 Computer Use 点击检测并读取结果。单次最多等待 60 秒，超时只刷新重试一次，不会无限等待，也不会使用 Codex 内置浏览器。该页提供的是区域指纹风险估计，不是 Claude 官方结论，也不检测 DNS、WebRTC 或 TUN；因此页面结果与原有网络验收都要通过。这是第三方页面；它当前声明检测信号只在本地处理，同时会加载 Google Analytics 统计匿名访问量。
