...
1. 先提醒用户：“请确保订阅开关已打开。”用户需自行登录服务商管理后台，打开每份远程订阅的订阅开关；部分服务的开关开启后约 10 分钟有效。收到“打开了”“没问题”或同义确认后才继续。
2. 为全部远程订阅建立清单、哈希和修改前备份。
3. 通过客户端界面一次触发“立即执行安全更新”；不得由代理登录服务商后台、检查或修改后台开关。
4. 逐份确认本轮刷新证据、YAML、Mihomo、代理组、当前策略和订阅自动更新仍关闭。
5. 一份失败时按平台事务规则保持整批一致，按订阅显示名称报告失败项，并优先提醒用户确认对应订阅开关仍在有效期内；订阅地址和节点名称保持隐藏。
