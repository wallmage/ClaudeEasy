...
2. 为全部远程订阅创建更新前备份。
3. 用 Computer Use 操作已经运行的 Clash 客户端，确认“自动更新”未勾选，再点击与用户手动更新相同的入口。
4. 客户端更新动作结束后立即结束。

ClaudeEasy 不再使用 `curl`、固定 User-Agent、本地控制器或配置文件替换来更新订阅。更新后不做防倒退或连通性等检查，不拒绝覆盖，不自动回滚，也不重新应用 Patch。

## 配置历史与恢复

第一次运行保存初始快照，每次写入前保存带日期时间的版本。比较只输出字段名和 SHA-256，不输出配置值。恢复前再次备份当前版本，并要求当前 SHA-256 仍等于比较时结果；并发变化、文件身份变化或未完成事务会阻止覆盖。

macOS 恢复当前订阅后通过本地控制器重新加载并验收；运行恢复失败时保留事务。Windows 受保护恢复只有客户端本来就未运行时进行。
