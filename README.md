...
首次结果会列出十项信号，权重依次为系统时区 24、浏览器语言 18、浏览器可见中文字体 14、国产厂商字体 10、WebRTC IP 泄露 10、国产浏览器/WebView 8、国产品牌设备 6、Intl 区域设置 4、UTC+8 偏移 3、Emoji 平台推断 3。除 WebRTC 外，项目、权重、分档、颜色和计分函数都与 FuckClaude 当前源码一致：0–30 分为绿色“低风险”，31–60 分为橙色“中等风险”，61–100 分为红色“高风险”。WebRTC 出口与同协议族的网页公网出口一致或没有暴露时为 0 分；两者不一致或暴露本地网络地址时为 10 分；无法取得网页出口或协议族无法对应时显示“无法读取”，不计成安全或泄露。语言项读取 `navigator.languages` 完整列表：大陆简体主语言计满分，后续出现大陆简体计 0.7，香港/澳门或繁体主语言计 0.5，后续其他中文计 0.4，台湾语言及其后随的通用 `zh` 回退不计分。系统时区、浏览器语言、Intl 区域设置和当前用于 Claude 的国产浏览器/WebView 都是可选调整，Skill 会把实际命中项、影响和建议值汇总后一次申请权限。你只回复“同意”且没有语言偏好时，默认使用繁体中文、台湾区域设置和台北时区；明确要求英文时，改用英文和美国区域设置，时区仍用台北。浏览器语言会把目标语言排在第一位。台北与北京、上海同为 UTC+8，不会改变当前时间；区域设置可能改变系统和其他应用的日期、数字格式。国产浏览器命中时只建议改用 Safari、Chrome 或 Edge 打开 Claude，不得仅为降低参考分修改系统默认浏览器；只有用户明确要求时才会更改默认浏览器，也不会卸载原浏览器。
