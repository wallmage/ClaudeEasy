...
4. macOS 在写入前完成 YAML、二次转换一致性检查和 Mihomo 校验。Windows 客户端刷新会运行已安装的全局脚本，按已保存档位重新打补丁；随后逐份确认本轮刷新并检查 YAML、代理组、Mihomo、全局脚本和运行配置，恢复并核对原 TUN 与代理选择；失败时恢复更新前备份并重新加载原运行配置，成功后再次确认订阅自动更新关闭。
...
macOS 安全更新恢复当前订阅后通过同一 ClashX Meta 进程的原生事件重新加载并验收；不会直接重载 Mihomo 或修改 TUN，运行恢复失败时保留事务且同一进程不重复加载。Windows 由 Clash Verge Rev 原生刷新；验收失败时从更新前备份恢复订阅，并重新加载原运行配置。其他受保护恢复只有客户端本来就未运行时进行。
