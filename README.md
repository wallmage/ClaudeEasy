...
ClaudeEasy 面向不熟悉电脑设置的普通用户。故障排查直接回答两件事：**Root Cause（原因）和 Fix Plan（修复方案）**，不用追问“为什么”。查不清时会说明具体缺口和下一项验证；修复请求会继续执行可安全完成的修复，并报告实际改动与复测结果。故障自然恢复不会被当成修复成功。网络配置、订阅检查与更新仍报告变化明细和处理结果。回复使用简体中文，隐藏敏感值和排查流水账。只有系统授权、密码或确实必须手动点击时才会请用户做一步操作。
