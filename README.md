...
档位 3 会在修改前后各用一次实际打开 Claude 的浏览器运行随 Skill 安装的离线检测页 `assets/claude-region-check.html`；无法确认实际浏览器时才使用系统默认浏览器并说明限制。页面用 CSP 禁止联网，正式支持 macOS 的 Safari、Chrome 和 Windows 的 Edge、Chrome。其他浏览器能运行就记录实际检测结果；不能运行就保持“未验证”。不得借用其他浏览器的信号，也不得合成一个不存在的指纹。Safari 缺少 Chromium 设备提示时会标明“有限信息”；浏览器没有提供的值显示“无法读取”，不会伪装成零命中。单次最多等待 60 秒，超时只刷新重试一次；文件缺失、不可读或页面报错时记为“未验证：本地检测页不可用”。不会无限等待，也不会使用 Codex 内置浏览器。
...
页面的 100 分是改编自上游规则的区域信号参考分，只用于比较修改前后；任何非零项都会单独列出，不能作为 Claude 是否可用的通过条件，也不替代 DNS、WebRTC、分流和 Claude 联网验证。浏览器可见中文字体、国产厂商字体、设备品牌、UTC+8 偏移和 Emoji 平台推断都会保留，不会为了降分删除字体、伪装设备、改变实际时钟或修改默认浏览器。可选调整前会明确说明“只降低参考分，不保证改变 Claude 判定”，并保存修改前快照、展示旧值→新值；本批设置或最终验收失败时，只在当前值仍等于本轮写入值时恢复，不得覆盖用户期间继续修改的值。复测仍使用实际打开 Claude 的同一浏览器，但不会根据参考分宣布档位 3 成功或失败。
