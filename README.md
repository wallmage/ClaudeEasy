...
- 三个档位的安装程序都检查 Mihomo，并把共同国内域名直连基线写入当前存储位置中的全部订阅。普通安装也先生成并校验整批候选；任一份失败时整批不写。档位 1、2 不增加 TUN、IPv6、WebRTC 或 AI 分组设置，也不改自动更新。通过基础环境检查并建立初始快照后，macOS 先把用户明确选择的新档位保存为恢复意图；用途档位通过私有临时文件替换，并同步文件和父目录，任一步失败都会恢复旧档位。随后才修改自动更新和订阅。普通失败会恢复旧档位，进程被强制结束时保留新选择，下一次 Patch 或安全更新按它完成未完成的配置，不能退回旧档位继续。档位 3 额外把 ClashX Meta 的 `kAutoUpdateEnable` 设为 `0`，回读确认后才继续，并记录仅用于撤销这次改动的所有权状态；后续步骤失败或安全卸载时恢复并再次回读原状态。原本已经关闭、用户后来已经恢复或没有所有权记录时不会擅自改写。
...
- 档位 3 重试会直接续写已有的 `prepared`/`installed` 恢复记录，不会先恢复为开启、释放所有权再重新关闭。由档位 1/2 升档时，关闭自动更新的子进程启动前就建立回退责任并暂存信号；失败时先恢复自动更新，再恢复旧档位。恢复自动更新失败时保留已同步的档位 3 并返回 `partial`，供下一次按档位 3 继续。档位 1/2 每次写订阅前还会清理旧版本遗留的自动更新所有权，清理失败则停止，不继续处理订阅文件。
...
- 客户端正在运行时，档位 1、2 只更新全局脚本，不碰 `verge.yaml`、`config.yaml` 或当前运行配置；补丁内容等待各订阅随后正常加载或刷新时生效，此时只能写“已更新，尚未生效”。档位 3 必须同时关闭当前会话内存中的订阅自动更新，单改磁盘上的 `profiles.yaml` 可能继续自动刷新或在客户端退出时被旧内存覆盖，因此返回 `client_running_profile_three_deferred`，不改用途档位、所有权、脚本或客户端配置，等客户端未运行时重试。
