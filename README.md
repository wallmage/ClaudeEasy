...
### 档位 3 的 Claude 区域指纹验收

档位 3 会在修改前后各用一次系统默认浏览器打开随 Skill 安装的离线检测页 `assets/claude-region-check.html`，由 Computer Use 点击检测并读取结果。页面不联网，正式支持 macOS 的 Safari、Chrome 和 Windows 的 Edge、Chrome；其他默认浏览器能正常完成初检就照常记录，不能运行时先用平台优先的受支持浏览器补测其余八项，再用当前默认浏览器的浏览器项替换补测结果中的对应项，重新计算总分和风险档，并把兼容性限制纳入一次授权。Safari 无法提供 Chromium 的设备提示时会标明“有限信息”，但仍能完成九项检测；本页只评估当前浏览器向网站暴露的信号，浏览器没有暴露的信息不会加分。单次最多等待 60 秒，超时只刷新重试一次；文件缺失、不可读或页面报错时记为“未验证：本地检测页不可用”。不会无限等待，也不会使用 Codex 内置浏览器。该页提供的是区域指纹风险估计，不是 Claude 官方结论，也不检测 DNS、WebRTC 或 TUN；因此页面显示绿色“低风险”（0–30）且原有网络验收通过，档位 3 才算完成。20 分左右只是理想参考，不能写死成门槛。

首次结果会列出九项信号。系统时区、浏览器语言、Intl 区域设置和当前国产浏览器/WebView 可以修改，Skill 会把实际命中项、影响和建议值汇总后一次申请权限。你只回复“同意”且没有语言偏好时，默认使用繁体中文、台湾区域设置和台北时区；明确要求英文时，改用英文和美国区域设置，时区仍用台北。浏览器语言会把目标语言排在第一位，并移除 `zh-CN`、`zh-Hans` 等简体中文语言项。台北与北京、上海同为 UTC+8，不会改变当前时间；区域设置可能改变系统和其他应用的日期、数字格式。国产浏览器命中时，只调整当前检测所用的默认浏览器：macOS 使用 Safari 或 Chrome，Windows 使用 Edge 或 Chrome，不卸载原浏览器；语言和区域设置会应用到最终用于复测的浏览器。

中文字体、国产厂商字体、设备品牌、UTC+8 偏移和 Emoji 风格会保留，不会为了降分删除字体、伪装设备或改变实际时钟。完成授权修改和全部网络补丁后，Skill 会用修改后的系统默认浏览器重新扫描；默认浏览器发生变化时，以新浏览器的结果为准。当前页面的绿色“低风险”区间是 0–30，20 分左右是理想参考而非固定门槛；只有页面显示绿色“低风险”，并且 DNS、WebRTC、分流和 Claude 联网验证也通过，档位 3 才算完成。
